...
{
  np_ = indata_.npart;
  JJ_ = indata_.totalMomentum;
  parity_ = indata.parity;
  if(parity_!= 1 && parity_ != -1)
  {
    std::cerr << "Problem with the parity, must be +/- 1" << std::endl;
    abort();
  }
  ...
  // If only J and parity is conserved
  if(indata_.ifJJCoupling)
  {
    double smax = 0.5*np_;
    while(smax>=0)
    {
      std::vector<int> sl; 
      for(int l=smax+JJ_; l>= std::abs(smax-JJ_); l--)
      {
        if(np_==2 && parity_ != pow(-1,l))
          continue;
        sl.push_back(l);
      }
      SL_.insert(std::pair<double,std::vector<int> >(smax,sl));
      smax--;
    }
    int lmax = 0.5*np_+JJ_;
    while(lmax>=0)
    {
      if(np_==2 && parity_ != pow(-1,lmax))
      {
        lmax--;
        continue;
      }
      std::vector<double> ls;
      for(double s=std::min(lmax+JJ_,0.5*np_); s>=std::abs(lmax-JJ_); s=s-1)
        ls.push_back(s);
      LS_.insert(std::pair<int,std::vector<double> >(lmax,ls));
  ...
  // prepare the eigenspace related to the most relevant eigenvalues
  uij_ = Eigen::MatrixXd::Zero(maxDimension,maxDimension);
  ...
  // set the initial dimension of the basis and the "principal" dimension
  k_=0;
  j_=0;
  ...
  for(int i=0; i<nofpairs; i++)
  {
    for(int j=0; j<(int)indata.alphaRanges[0].size()/2; j++)
    {
      ...
      //b = pow(b,nofpairs);
      //r_[j] = std::uniform_real_distribution<double> (a,b);
      //double mean = (a+b)/2;
      //dist_[j].push_back(std::normal_distribution<double> (0,1));
...
//****
//TEST ELEMENT TO FIND THE FIRST EIGENVALUE EFFICIENTLY (S-V pg 29)
//****


double Basis::find_lowest_eig_efficiently_ (Eigen::MatrixXd h) {
	...
	if (k_ == 0) {
		...
		if (energy_(0,0) > 0) 	x_lo = energy_(0,0) -  2*energy_(0,0);
		else if (energy_(0,0) < 0)  x_lo = energy_(0,0) +  2*energy_(0,0);

		x_hi = energy_(0,0); 
	
  		gsl_function F;
		struct my_f_params params = {k_, h, energy_};

  		F.function = &func_to_find_root_;
	  	F.params = &params;
	  	...
	  	do{
			...
			cout << "Converged:" << endl;
      			...
      			cout << "#of iteration:  " << iter << "  Lower:  " << x_lo <<  "  Upper:  " <<  x_hi <<  "  Result at this iter:  " << r  << "   " << x_hi - x_lo << endl;
...
/*
std::tuple<Eigen::VectorXd, //should contain the first eigenvalue
           std::pair<Eigen::MatrixXd,Eigen::VectorXd>, // <a,u>
           Eigen::VectorXd,  // alphas
           Eigen::VectorXd, // spin
           Eigen::VectorXd> // isospin
Basis::testElementEfficient()
{
  ...
  Eigen::VectorXd eigenvaluesEff = Eigen::VectorXd::Zero(maxDimension_);
  ...
  double eig = find_lowest_eig_efficiently_(h);

  eigenvaluesEff = energy_.col(0);

  eigenvaluesEff.coeffRef(0) = eig;

  std::cout << "The first Eig in Test El Eff is: \n " <<  eigenvaluesEff(0) << std::endl;

  //eigenvaluesEff = energy_.col(0);

  //eigenvalues = energy_.col(0);
  ...
  auto out =
    std::make_tuple(eigenvaluesEff,std::make_pair(a,u),alpha,lS,lI);
  return out;
} */
...
Eigen::MatrixXd Basis::PseudoInvGIJ() {

	  /*std::cout << "Overlaps before PseudoINv calc: \n" << gij_.block(0,0,k_+1,k_+1) << std::endl;
	  std::cout << "****************" << std::endl;*/
			  
	  Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> svdofG(gij_.block(0,0,k_+1,k_+1));
  
          Eigen::VectorXd singValOfG = svdofG.eigenvalues();

	  /*std::cout << "Singval G: \n" << singValOfG << std::endl;
          std::cout << "****************" << std::endl;*/

          Eigen::MatrixXd SinvG(k_+1,k_+1);
          SinvG = Eigen::MatrixXd::Zero(k_+1,k_+1);

          for (int i = 0; i < k_+1; i++) {
                  if (abs(singValOfG(i)) < 1e-4) SinvG(i,i) = 0.;
                  else SinvG(i,i) = 1.0/singValOfG(i);
          }
          
	  Eigen::MatrixXd PinvG(k_+1,k_+1);

	  PinvG = svdofG.eigenvectors()*SinvG*svdofG.eigenvectors().transpose();
...
Basis::testElement()
{
  ...
  Eigen::VectorXd sigma(nofpairs);
  Eigen::VectorXd sigma_alpha(nofpairs);
  //double norm_sigma_dist = 0;
  //std::cout << "State: " << state_ << std::endl;
  for(auto i=0; i< nofpairs; i++) {
    sigma(i) = dist_[state_][i](mt_);
    //std::cout << "Random sigma from norm dist: " << sigma(i) << std::endl;
    //norm_sigma_dist += pow(sigma(i),2);
    alpha(i) = 1.0/pow(sigma(i),2);
  }
  //norm_sigma_dist = sqrt(norm_sigma_dist);
  //for(auto i=0; i< nofpairs; i++) {
    //double random_rad = r_[state_](mt_);
    //std::cout << "Random radIus from uniform dist: " << random_rad << std::endl; 
    //double rad = pow(random_rad,1./nofpairs);
    //std::cout << "Random radius from uniform dist: " << rad << std::endl;
    //sigma_alpha(i) = abs(rad*sigma(i)/norm_sigma_dist);
    //std::cout << "Random distances: " << sigma_alpha(i) << std::endl;
    //alpha(i) = 1.0/pow(sigma_alpha(i),2);
    //alpha(i) = 1.0/pow(sigma(i),2);
  //}
...
*/
  ...
  /*std::cout << "*******The hamil in testEl ******** \n " << h << std::endl;
  std::cout << "*************" << std::endl;
  //std::cout << "The overlaps at dimension " << k_ << " is \n" << gij_.block(0,0,k_+1,k_+1) << std::endl;
  //std::cout << "*************" << std::endl;*/
  //Eigen::MatrixXd PinvG = PseudoInvGIJ();

  /*std::cout << "******The PinvG in testEl********** \n" << PinvG << std::endl;
  std::cout << "*************" << std::endl;*/

  Eigen::BDCSVD<Eigen::MatrixXd> svd(gij_.block(0,0,k_+1,k_+1),Eigen::ComputeFullU);

  //std::cout << svd.singularValues()  << std::endl;

  double eps_thresh = indata_.SVDthreshold;

  for(j_=k_; svd.singularValues()(j_) < eps_thresh ; j_--);
  
  // Eigen::MatrixXd Ut(k_+1,j_+1);
  uij_ = svd.matrixU().block(0,0,k_+1,j_+1);
  
  Eigen::MatrixXd Gt(k_+1,j_+1);
  Gt = gij_.block(0,0,k_+1,k_+1)*uij_;

  Eigen::MatrixXd Ht(k_+1,j_+1);
  Ht = h.block(0,0,k_+1,k_+1)*uij_;

  Eigen::MatrixXd pinvGt = Gt.completeOrthogonalDecomposition().pseudoInverse();

  Eigen::EigenSolver <Eigen::MatrixXd> es(pinvGt*Ht,Eigen::EigenvaluesOnly);
//  std::cout << "The eigenvalues of Pinv h are:" << std::endl << es.eigenvalues().real() << std::endl;
  // std::cout << "The eigenvectors of h are:" << std::endl << es.eigenvectors() << std::endl;
  ...
  */
  ...
  //A.push_back(a);
  //k_++;
 /* std::cout << "The ham in test El: " << std::endl;
  std::cout << h << std::endl;
  std::cout << "The eigenvalues std way: " << std::endl;
  std::cout << es.eigenvalues() << std::endl;*/


  std::vector<double> eigenvals;
  for (int i = 0 ; i<j_+1; i++) eigenvals.push_back(es.eigenvalues().real()(i));
  std::sort(eigenvals.begin(),eigenvals.end());
  
  Eigen::VectorXd sorted_eigenvals(j_+1);

  for (int i = 0 ; i<j_+1; i++) sorted_eigenvals(i) = eigenvals.at(i);

  auto out =
    std::make_tuple(sorted_eigenvals,std::make_pair(a,u),alpha,lS,lI);
  return out;
}
...
/*  std::cout << "*****The Ham in newEl****************" << std::endl;
  std::cout << " h = " << std::endl << h << std::endl;
  std::cout << "*************************************" << std::endl;
  std::cout << "*****The overlap in newEl****************" << std::endl;
  std::cout << " Original Overlap g = " << std::endl << gij_.block(0,0,k_+1,k_+1) << std::endl;
  std::cout << "*************************************" << std::endl;*/
...
//  std::cout << "*****The Ham Proj in newEl****************" << std::endl;
//  std::cout <<  Ht << std::endl;
//  std::cout << "*************************************" << std::endl;
//  std::cout << "*****The overlap Proj in newEl Gt ****************" << std::endl;
//  std::cout << Gt << std::endl;
//  std::cout << "*************************************" << std::endl;	  
//  std::cout << "******The PinvG in newEl********** \n" << pinvGt << std::endl;
//  std::cout << "*************" << std::endl;  
  //std::cout << "The eigenvalues of h are:" << std::endl << es.eigenvalues() << std::endl;
  // std::cout << "The eigenvectors of h are:" << std::endl << es.eigenvectors() << std::endl;
  ...
  Eigen::EigenSolver<Eigen::MatrixXd> es(pinvGt*Ht);
  ...
  /*std::cout << "******The  EigVecs in newEl Before sorting********** \n" << eigen_vectors << std::endl;
  std::cout << "*************" << std::endl;
  std::cout << "******The  EigVals in newEl Before sorting********** \n" << eigen_values << std::endl;
  std::cout << "*************" << std::endl;*/
  ...
  std::vector<std::tuple<double, Eigen::VectorXd>>::iterator it;
  Eigen::VectorXd eigen_vals(j_+1);
  Eigen::MatrixXd eigen_vecs(j_+1,j_+1);
  for (int i = 0 ; i<j_+1; i++) {
  	eigen_vals(i) = std::get<0>(eigen_vectors_and_values.at(i));
        eigen_vecs.col(i) = std::get<1>(eigen_vectors_and_values.at(i));
  }

  // the new c_ij are
  cij_.block(0,0,j_+1,j_+1) =  eigen_vecs;
//  std::cout << "******The  EigVecs in newEl After Sorting Cij ********** \n" << eigen_vecs << std::endl;
//  std::cout << "*************" << std::endl;
 // std::cout << "******The  EigVals in newEl After Sorting********** \n" <<  eigen_vals << std::endl;
 // std::cout << "*************" << std::endl;
//  std::cout << "******The  principal EigSpace Uij ********** \n" <<  uij_.block(0,0,k_+1,j_+1) << std::endl;
//  std::cout << "*************" << std::endl;
 // std::cout << "******The  Sqaure Radius Matrix********** \n" <<  raij_.block(0,0,k_+1,k_+1) << std::endl;
 // std::cout << "*************" << std::endl;

  // the energy
  energy_.block(0,0,j_+1,1) = eigen_vals;
...
Basis::refineSimplex(int startingPoint)
{
           ...
           aold != std::prev(Au_.end(),1); aold++)
  {
    ...
    Eigen::GeneralizedEigenSolver <Eigen::MatrixXd> 
      es((haij_+vaij_).block(0,0,k_,k_),gij_.block(0,0,k_,k_));
    cij_.block(0,0,k_,k_) =  es.eigenvectors().real();
    ...
    Eigen::VectorXd &lS, Eigen::VectorXd &lI)
{
  ...
  std::cout << "The eigenvecs in calculate hamilt after Diag: \n" << cij_.block(0,0,k_,k_) << std::endl; */
...
Basis::evaluatePotential_(int pair, 
    ...
    Eigen::VectorXd lpS, Eigen::VectorXd lS, Eigen::VectorXd lpI, Eigen::VectorXd lI, Eigen::MatrixXd A,
    Eigen::MatrixXd A1, int kind)
{
  ...
  switch(kind)
  {
    ...
    case InputData::MINNESOTA:
    {
      ...
      // new singlet channel
      /*
      double v3 = -97.45/indata_.energyUnits;
      double r3 = 1/sqrt(0.48);
      */
    ...
    {
      double v0 = indata_.twoBodyParameters[pair][1]/indata_.energyUnits;
      double r0 = indata_.twoBodyParameters[pair][0]/indata_.lengthUnits;
      double partialpot =0;
      for(int n=0; n<=L_; n++)
      {
        partialpot += 1./fact(L_-n)
    ...
    case InputData::TWOGAUSSIAN_BARRIER:
    {
      ...
      double v1 = indata_.twoBodyParameters[pair][2]/indata_.energyUnits;
      double r1 = indata_.twoBodyParameters[pair][3]/indata_.lengthUnits;

      double a = indata_.twoBodyParameters[pair][4]/indata_.lengthUnits;
      ...
      for(int n=0; n<=L_; n++)
      {
        partialpot += 1./fact(L_-n)
          * pow(gamma*gammap/c/rho,n)
          * shiftedGaussian_(n,c,v1,r1,a);
      }
      value += partialpot*fact(L_)*norma;


    }
    break; // twogaussian_barrier

    case InputData::SHIFTED_GAUSSIAN:
    {
      double v1 = indata_.twoBodyParameters[pair][0]/indata_.energyUnits;
      double r1 = indata_.twoBodyParameters[pair][1]/indata_.lengthUnits;

      double a = indata_.twoBodyParameters[pair][2]/indata_.lengthUnits;
    ...
    break; // shifted_gaussian

    case InputData::COULOMB_REG:
    {
    ...
    break; // Coulomb Regularized

    case InputData::ALPHA_ALPHA: 
    {
      double v0 = indata_.twoBodyParameters[pair][1]/indata_.energyUnits;
      double v1 = indata_.twoBodyParameters[pair][2]/indata_.energyUnits;
      double r0 = indata_.twoBodyParameters[pair][0]/indata_.lengthUnits;
      ...
      for(int n=0; n<=L_; n++)
      {
        partialpot += 1./fact(L_-n)
          * pow(gamma*gammap/c/rho,n)
          * (gaussian_(n,c,v0,r0) + r2_gaussian_(n,c,v1,r0));
      }
      value = partialpot*fact(L_)*norma;
    }
    break; // Alpha_Alpha Master Thesis 2014-2015 Paolo Recchia (only local) 
	

    case InputData::YAMAGUCHI:
    {
	 Erfc erfc;
         Eigen::MatrixXd Tinv = coo_.transpositionJacobi[pair].inverse();
	 Eigen::MatrixXd TAT = Tinv.transpose()*A*Tinv;
         Eigen::MatrixXd TAiT = Tinv.transpose()*A1*Tinv;
         Eigen::MatrixXd C =  TAT.bottomRightCorner(np_-2,np_-2);
         Eigen::MatrixXd Ci =  TAiT.bottomRightCorner(np_-2,np_-2);
         Eigen::VectorXd b = TAT.col(0).tail(np_-2);
         Eigen::VectorXd bi = TAiT.col(0).tail(np_-2);
         double b1 = TAT(0,0);
         double b1i = TAiT(0,0);  
         double epsilon = 0.5*b1 - 0.5*b.transpose()*C.inverse()*b;
         double epsiloni = 0.5*b1i - 0.5*bi.transpose()*Ci.inverse()*bi;
         double alphaYama = 0.2316;
         double betaYama = 6.255*alphaYama;
         double lambdaYama = betaYama*pow((alphaYama + betaYama)/M_PI,2);
         
         value = -lambdaYama*pow(M_PI,2)/2./epsilon/epsiloni*pow(2.*M_PI,3*(np_-2.))/pow(sqrt(C.determinant()*Ci.determinant()),3)*
                  (1. - betaYama/2.*sqrt(M_PI/epsilon)*exp(betaYama*betaYama/4./epsilon)*erfc(betaYama/2./sqrt(epsilon)))*
                  (1. - betaYama/2.*sqrt(M_PI/epsiloni)*exp(betaYama*betaYama/4./epsiloni)*erfc(betaYama/2./sqrt(epsiloni)));

    } break;  // No local Yamaguchi Separable Potential Yamaguchi, Yoshio. 
...
double
Basis::r2_gaussian_(int n,double c, double v0, double r0)
{
      // n = 3; Integrate[(1/Sqrt[Pi]/(2*n+1)!)*2/c*x^2*Exp[-2*(x^2/c/r0^2) - x^2]*
      //   HermiteH[1, x]*HermiteH[2*n + 1, x], {x, 0, Infinity}, 
      //    Assumptions -> {c > 0, r0 > 0}]
  double value=0;
  double k = 2./c/r0/r0 + 1;
  switch(n)
  {
    case 0:
    {
      value = (3.0*v0/c/r0/r0)/pow(k,2.5);
    }
    break;

    case 1:
    { 
      value = (v0/c/r0/r0)*(5.0-3.0*k)/pow(k,3.5);
    }
    break;

    case 2:
    { 
      value = (0.5*v0/c/r0/r0)*(k-1.)*(3.*k-7.)/pow(k,4.5);
    }
    break;
  }
...
double
Basis::r4_gaussian_(int n,double c, double v0, double r0)
{
      // n = 3; Integrate[(1/Sqrt[Pi]/(2*n+1)!)*x^4*Exp[-2*(x^2/c/r0^2) - x^2]*
      //   HermiteH[1, x]*HermiteH[2*n + 1, x], {x, 0, Infinity}, 
      //    Assumptions -> {c > 0, r0 > 0}]
  double value=0;
  double k = 2./c/r0/r0 + 1;
  switch(n)
  {
    case 0:
    {
      value = (15.0*v0/c/c)/pow(k,3.5);
    }
    break;

    case 1:
    { 
      value = (5.*v0/c/c)*(7.-3.0*k)/pow(k,4.5);
    }
    break;

    case 2:
    { 
      value = (0.5*v0/c/c)*(63.+5.*k*(3.*k-14.))/pow(k,5.5);
    }
    break;
  }
...
double
Basis::shiftedGaussian_(int n,double c, double v1, double r1, double a)
{
  Erf erf;
  double value=0;
  switch(n)
  {
    ...
    {
      value = v1/sqrt(M_PI) * 
      (pow(c,1.5)*(4*a*r1*sqrt(2/c + pow(r1,2)) + 
       (pow(M_E,(2*pow(a,2))/(pow(r1,2)*(2 + c*pow(r1,2))))*sqrt(2*M_PI)*
          (4*pow(a,2) + 2*pow(r1,2) + c*pow(r1,4))*
          (1 + erf((sqrt(2)*a)/(r1*sqrt(2 + c*pow(r1,2))))))/sqrt(c)))/
   (pow(M_E,pow(a,2)/pow(r1,2))*sqrt(2 + 4/(c*pow(r1,2)))*
     pow(2 + c*pow(r1,2),2));
    }
    break;
  }
...
double
Basis::coul_reg_(int n,double c)
{
      // n = 3; Integrate[(1/Sqrt[Pi]/(2*n+1)!)*sqrt[c/2]/x*erf[sqrt[2/c]*x]*Exp[- x^2]
      //   HermiteH[1, x]*HermiteH[2*n + 1, x], {x, 0, Infinity},
      //    Assumptions -> {c > 0}]
  double value=0;
  switch(n)
  {
    case 0:
    {
      value = 1/2*sqrt(c/(c+2));
    }
    break;

    case 1:
    {
      value = 1/2*sqrt(c/(c+2)) + 2/3*c/(c+2)*atan(sqrt(2/c));
    }
    break;

    case 2:
    {
      value = 24*pow(c/(2+c),2.5);
    }
    break;
  }
...
Eigen::VectorXd &lS, Eigen::VectorXd &lI,int sign)
{
  ...
  for(auto aui : Au_)
  {
   ...
   //std::cout << "S_" << indx_new << "," << indx_i << "=" << gij_(indx_new,indx_i) << std::endl;
    ...
    /*
     * No local Alpha-Alpha (Paolo Recchia Master Thesis 2014-2015)
     */

    /*  
    if(indata_.ifNoLocalAlpha) {
	   double aN = coo_.zita[i].transpose()*a*coo_.zita[i];
	   double aiN = coo_.zita[i].transpose()*aui.first*coo_.zita[i];
	   Eigen::MatrixXd Jinv = binv - cij*(binv*coo_.W.col(i)*coo_.W.row(i)*binv);
	   Eigen::VectorXd etaN = Jinv*a*coo_.zita[i];
	   Eigen::VectorXd etaiN = Jinv*aui.first*coo_.zita[i];
	   double alphaN = aN - coo_.zita[i].transpose()*a*etaN;
	   double alphaiN = aiN - coo_.zita[i].transpose()*aui.first*etaiN;
	   Eigen::VectorXd tmpN = a*etaiN + aui.first*etaN;
	   double beta = -0.5*coo_.zita[i].transpose()*tmpN;
	   Eigen::VectorXd nuN = coo_.zita[i] - etaN - etaiN;
	   Eigen::VectorXd muN = coo_.zita[i] - etaN + etaiN;
	   Eigen::VectorXd muiN = coo_.zita[i] - etaiN + etaN;


	   double RN = -(2.*beta + alphaN + alphaiN);
	   double RiN = -2.*beta + alphaN + alphaiN;
	   double PiN = aui.second.transpose()*aui.second*nuN.transpose()*nuN;
	   double PN = u.transpose()*u*nuN.transpose()*nuN;
	   double Q = 2.*aui.second.transpose()*u*nuN.transpose()*nuN;

	   double qtmp = 0.5*u.transpose()*Jinv*u;
	   double qitmp = 0.5*aui.second.transpose()*Jinv*aui.second;
	   double rhotmp = 0.5*aui.second.transpose()*Jinv*u;
	
	   double tmpVar = u.transpose()*u*muN.transpose()*muN;
	   double qN = qtmp + tmpVar/(2*RiN);
	   tmpVar = aui.second.transpose()*aui.second*muN.transpose()*muN;
       	   double qiN = qitmp + tmpVar/(2*RiN); 
	   tmpVar = aui.second.transpose()*u*muiN.transpose()*muN;
       	   double rhoN = rhotmp - tmpVar/RiN;


 	   double noLscalarProduct = scalConst/pow(det_b,1.5)*pow(rhoN,L_);

	   noLpot = (RN + L_*Q/rhoN)/2.*pow(2.*cij/RiN,1.5);
  }
  ...
  */
    ...
    {
      double tmp3Pot=0;
      double V0 = indata_.threeBodyParameters[1]/indata_.energyUnits;
      double r02 = indata_.threeBodyParameters[0]*indata_.threeBodyParameters[0]
    ...
    lpS++;
    lpI++;
  //std::cout << gij_.block(0,0,k_,k_) << std::endl;
...
Basis::saveAll(std::string fileName)
{
  ...
  /* Create the dataset. */
        dataset_id = H5Dcreate (file_id, "/vij", H5T_NATIVE_DOUBLE,
        dataspace_id, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT);

  matrix = gij_.block(0,0,k_,k_)*uij_.block(0,0,k_,j_)*cij_.block(0,0,j_,j_);
  /* Write the first dataset. */
   status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  /* Close the first dataset. */
...
/*
 * Load all the the data from a HDF5 file
 */
int 
Basis::loadAll(std::string fileName)
{
  // we open the hdf5 file 
  hid_t       file_id, dataset_id, dataspace_id;  /* identifiers */
  //hid_t     group_id , attribute_id; /* identifiers */
  hid_t       dspace_id;
  herr_t      status;
  ...
  */
  ...
  /*
   * Read the overlaps gij_
   */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/overlaps", H5P_DEFAULT); 
  ...
  k_ = dims[0];
  Eigen::MatrixXd matrix(k_,k_);

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  gij_.block(0,0,k_,k_) = matrix;

  /* Close the first dataset. */
   status = H5Dclose(dataset_id);

  /*
   * Read the kinetic energy haij_
   */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/kinetic", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  haij_.block(0,0,k_,k_) = matrix;

  /* Close the first dataset. */
  status = H5Dclose(dataset_id);

  /*
   * Read the two-body potential energy
   */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/twoBodyPotential", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  vaij_.block(0,0,k_,k_) = matrix;
 ...
 /*
  * Read the three-body potential energy
  */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/threeBodyPotential", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  taij_.block(0,0,k_,k_) = matrix;
  ...
  /*
   * Read the radius matrix energy raij_
   */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/radiusSquare", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  raij_.block(0,0,k_,k_) = matrix;
  ...
  status = H5Dclose(dataset_id);
	...
	/*
	 *  Read the harmonic trap energy
   */ 

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/harmonicTrap", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  trapij_.block(0,0,k_,k_) = matrix;

  /* Close the first dataset. */
  status = H5Dclose(dataset_id);

	/*
	 *  Read the four-body potential energy
   */ 

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/fourBodyPotential", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  ta4ij_.block(0,0,k_,k_) = matrix;
  ...
  /*
   * Read the three-body potential spin energy
   */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/threeBodyPotentialSpin", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  tapij_.block(0,0,k_,k_) = matrix;

  /* Close the first dataset. */
  status = H5Dclose(dataset_id);

  /*
   * Read the coulomb potential energy
   */

  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/coulombPotential", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  qaij_.block(0,0,k_,k_) = matrix;
  ...
  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/phiij", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  phiij_.block(0,0,k_,k_) = matrix;
  ...
  /* Open the dataset. */
  dataset_id = H5Dopen2 (file_id, "/cij", H5P_DEFAULT); 

  status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     matrix.data());
  cij_.block(0,0,j_,j_) = matrix;
...
/*
 * Write the Coupling Spin/L structure
 */
void
Basis::printCouplings(std::ostream& outFile)
{
  outFile << "Total Angular momentum J = " << JJ_ << std::endl;
  outFile << "=============================== " << std::endl;
  outFile << "      L             S         " << std::endl;
  outFile << "------------------------------" << std::endl;
  for (auto ls : LS_)
    for(auto e : ls.second)
  outFile << "      " << ls.first  << "            " << e << std::endl;
  outFile << "=============================== " << std::endl;
  outFile << "      S             L         " << std::endl;
  outFile << "------------------------------" << std::endl;
  for (auto ls : SL_)
    for(auto e : ls.second)
  outFile << "      " << ls.first  << "            " << e << std::endl;
