...
Basis
{
  ...
  private:
   ...
   int j_;  // the current "principal" dimension

   /*
    * This is for the JJ coupling 
    */
   double JJ_; // total angular momentum
   // for a given L the compatibles S
   std::map<int,std::vector<double> > LS_; 
   // for a given S the compatibles L
   std::map<double,std::vector<int> > SL_; 
  ...
  //std::vector< std::vector<std::normal_distribution<double> > > dist_;
  std::vector< std::vector<std::uniform_real_distribution<double> > > dist_;
  // this is for r (random radius for random normal dist) 
  std::vector<std::uniform_real_distribution<double> > r_;
  // this is for u
  ...
  std::uniform_real_distribution<double> isospinDist_;
   ...
   Eigen::MatrixXd noLocij_; //if no local term is considered
   // matrix of the 
   ...
   // The eigenspace related to the most relevant eigenvalues
   Eigen::MatrixXd uij_;
   ...
   double evaluatePotential_(int pair, double norma, double c, double rho,
     ...
     Eigen::MatrixXd A, Eigen::MatrixXd A1, 
     int kind);
   ...
   // just the gaussian integral (A.129) 
   // v0*exp( -r**2/r0**2 )
   double
   gaussian_(int n,double c, double v0, double r0);
   // just the gaussian multiplied by r^2 integral
   // v0*r**2*exp( -r**2/r0**2 )
   double
   r2_gaussian_(int n,double c, double v0, double r0);
   // just the gaussian multiplied by r^4 integral
   // v0*r^4*exp( -r**2/r0**2 )
   double
   r4_gaussian_(int n,double c, double v0, double r0);
   // the shifted gaussian (A.129)
   // v0*exp( -(r-a)**2/r0**2 )
   double
   shiftedGaussian_(int n,double c, double v0, double r0, double a);
   // the regularized coulomb
   // v0*r0/x*erf(x/r0)
   double
   coul_reg_(int n,double c);
   ...
   simplex_(
    ...
    double find_lowest_eig_efficiently_ (Eigen::MatrixXd h); //Find root of funcToFindRoot (should be the lowest eigenvalue)
  ...
  public:
    ...
    // Test a new element in a more Efficient way (S-V pg 29)
    std::tuple<Eigen::VectorXd, // eigenvalues
               std::pair<Eigen::MatrixXd,Eigen::VectorXd>, // <a,u>
               Eigen::VectorXd, // alphas
               Eigen::VectorXd,//spin
               Eigen::VectorXd> // isospin
    testElementEfficient();
    //Calculate the Pseudoinverse of the overlap matrix
    Eigen::MatrixXd PseudoInvGIJ();    
    ...
    Eigen::MatrixXd * getUIJ(void)           { return & uij_;};
    ...
    int               getPrincipalDimension(void)   { return j_;};


    /*
     * Just write the spin/L structure
     */
    void printCouplings(std::ostream& outFile);
    ...
    /*
     * Reload all of the data in a HDF5 file - very simple
     */
    int loadAll(std::string fileName);
};
